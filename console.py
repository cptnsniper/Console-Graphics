...
def draw_ray():
    ...
    for i in range(y):
        for j in range(x):
            ...
            hit = False
            for ii in range(10):
                ray = (u * 10, v * 10, sphere_dist(ray, sphere_origin, 5))
                if (sphere_dist(ray, sphere_origin, 5) < 1):
                    hit = True
                    break
            if (hit):
                pixel = shades[round(map(angle(subtract(ray, sphere_origin), light), 0, pi, 0, len(shades) - 1))]
                # break
            draw_string += pixel
